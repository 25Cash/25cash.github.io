...
if (isset($_GET['mod'])) {
    $view = $_GET['mod'];
    $select = "SELECT * FROM donation WHERE ot_id='$view'";
...
<div class="main">
        ...
        <div class="bottom">
            ...
            if (isset($_POST['update'])) {
                ...
                $update = "UPDATE donor SET fname= '$_lname',lname= '$_lname',gender= '$_gender',date='$_date',email= '$_email' WHERE d_id = '$_id'";
                $runupdate = mysqli_query($con,$update);
                if ($update) {
                    echo "<script>window.location.replace('donors.php')</script>";
                }
                else{
                    echo "<script>alert('uri mubiki?')</script>";
                }
            }
            ?>
        </div>
