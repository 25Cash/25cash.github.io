    ...
    <div class="main">
        ...
        <form class="bottom" method="POST">
            ...
            <div class="courses">
                <div class="software">
                    <?php 
                        $sod = mysqli_query($con,"SELECT * FROM courses WHERE domain='software_development'");
                        if (mysqli_num_rows($sod) > 0) {
                            while ($fetch_software = mysqli_fetch_assoc($sod)) {
                    ?>
                    <div class="choice">
                        <span><p><?=$fetch_software['course_name']?></p><input type="checkbox" name="course" id="" value="html"></span>
                    </div>
                    <?php } }?>
                </div>
                <div class="capp">
                    <?php 
                        $coa = mysqli_query($con,"SELECT * FROM courses WHERE domain='computer_application'");
                        if (mysqli_num_rows($coa) > 0) {
                            while ($fetch_computer_application = mysqli_fetch_assoc($coa)) {
                    ?>
                    <div class="choice">
                        <span><p><?=$fetch_computer_application['course_name']?><input type="checkbox" name="course" id="" value="graphics_design"></span>
                    </div>
                    <?php } }?>
                </div>
                <div class="cst">
                    <?php 
                        $csy = mysqli_query($con,"SELECT * FROM courses WHERE domain='computer_system'");
                        if (mysqli_num_rows($csy) > 0) {
                            while ($fetch_computer_system = mysqli_fetch_assoc($csy)) {
                    ?>
                    <div class="choice">
                        <span><p><?=$fetch_computer_system['course_name']?></p><input type="checkbox" name="course" id="" value="troubleshooting"></span>
                    </div>
                    <?php } }?>
                </div>
                <div class="mas">
                    ...
                    ?>
                    <div class="choice">
                        <span><p><?=$fetch_masonry['course_name']?></p><input type="checkbox" name="course" id="" value="workshop"></span>
                    </div>
                    <?php } }?>
                </div>
                <div class="rct">
                    ...
                    ?>
                    <div class="choice">
                        <span><p><?=$fetch_road_construction['course_name']?></p><input type="checkbox" name="course" id="" value="drainage"></span>
                    </div>
                    <?php } }?>
                </div>
            </div>
